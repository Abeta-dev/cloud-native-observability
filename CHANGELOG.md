...
### Added
- architecture: Published clustered observability architecture guide (`docs/CLUSTERED_OBSERVABILITY.md`) detailing high-availability Grafana Mimir deployment, multi-tenant metric aggregation, and remote-write buffer tuning.
- docker-compose: Added `deploy/docker-compose/mimir-remote-write.yaml` overlay configuring Prometheus remote-write streaming to enterprise Mimir endpoints.

### Changed
- docker-compose: Purged deprecated `version: "3.8"` attributes across all docker compose stacks to comply with Compose V2 specification.
